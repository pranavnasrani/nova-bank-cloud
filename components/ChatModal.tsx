...
if (recognition) {
    ...
    // The language is set dynamically in the component's useEffect
...
export const ChatModal: React.FC<ChatModalProps> = ({ isOpen, onClose }) => {
  const { currentUser, transferMoney, contacts, addCardToUser, addLoanToUser, requestPaymentExtension, transactions } = useContext(BankContext);
  ...
  const contactNames = contacts.filter(u => u.id !== currentUser?.id).map(u => u.name);

  useEffect(() => {
    if (isOpen && currentUser) {
        setMessages([{ id: messageId.current++, sender: 'ai', text: t('chatGreeting', { name: currentUser?.name.split(' ')[0] })}]);
        setInputValue('');
        setChat(createChatSession(currentUser.name, contactNames, language));
    } else {
        setChat(null);
    }
  ...
  const handleImageFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
        ...
        // Use the most specific identifier available for the payment prompt
      ...
      if (functionCalls && functionCalls.length > 0) {
        ...
        for (const call of functionCalls) {
            ...
            if (call.name === 'initiatePayment') {
                ...
                const result = await addCardToUser(call.args.applicationDetails as CardApplicationDetails);
                resultMessage = result.message;
                resultForModel = result;
                ...
                // FIX: Passed the 'language' argument to `analyzeSpendingWithAI` to match its function signature and resolve the error.
