...
// FIX: Explicitly typed TransactionItem as a React.FC to ensure it's recognized as a component that accepts a 'key' prop in lists.
...
export const HomeScreen = () => {
    ...
    useEffect(() => {
        const fetchSpendingAnalysis = async () => {
            ...
            const now = new Date();
            const thisMonthTransactions = allUserTransactions.filter(tx => {
                const txDate = new Date(tx.timestamp);
                return txDate.getMonth() === now.getMonth() && txDate.getFullYear() === now.getFullYear();
            });

            const analysisResult = await analyzeSpendingWithAI(thisMonthTransactions, language);
